...
class FourierImageAnimation {
    ...
    getCubeVertices() {
        ...
        const projected = vertices3D.map(v => {
            // Rotate in 3D space
            const rotated = window.rotate3D ? window.rotate3D(v[0], v[1], v[2], this.cubeRotation.A, this.cubeRotation.B, this.cubeRotation.C) : this._rotate3DFallback(v[0], v[1], v[2], this.cubeRotation.A, this.cubeRotation.B, this.cubeRotation.C);
            const x = rotated.x;
            const y = rotated.y;
            const z = rotated.z;
    ...
    // Fallback rotate3D if utility not loaded
    _rotate3DFallback(x, y, z, A, B, C) {
        let x1 = x;
    ...
    // Legacy functions for compatibility (using shared rotate3D utility)
    calculateX(i, j, k, A, B, C) {
        const rotated = window.rotate3D ? window.rotate3D(i, j, k, A, B, C) : this._rotate3DFallback(i, j, k, A, B, C);
        return rotated.x;
    }
    ...
    // Use shared convexHull utility
    convexHull(points) {
        return window.convexHull ? window.convexHull(points) : this._convexHullFallback(points);
    }
    
    // Fallback convexHull implementation
    _convexHullFallback(points) {
    ...
    resamplePoints(pts, N) {
        if (window.resamplePoints) {
            return window.resamplePoints(pts, N);
        }
        // Fallback implementation
        return this._resamplePointsFallback(pts, N);
    }
    
    _resamplePointsFallback(pts, N) {
    ...
    dft(input) {
        if (window.dft) {
            return window.dft(input);
        }
        // Fallback implementation
        return this._dftFallback(input);
    }
...
// NavCubeAnimation has been moved to animations/nav-cube-animation.js
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Navigation cube animation is initialized in nav-cube-animation.js
});
